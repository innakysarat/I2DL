...
class Classifier(Network):
    ...
    def initialize_weights(self, weights=None):
        ...
        if weights is not None:
            ...
            # a vector in the shape (num_features + 1, 1) with mean = 0 and standard deviation = 0.001
    ...
    def forward(self, X):
        ...
        ########################################################################
        # Affine step: linear transformation
        z_affine = np.dot(X, self.W)

        # Apply sigmoid activation
        z = self.sigmoid(z_affine)
    ...
    def backward(self, dout):
        ...
        X, z = self.cache  # X includes the bias term, z is the sigmoid output

        # Gradient of sigmoid = dL/dz * z'(s), s = X * w
        # sigma'(z) = sigma(z) * (1 - sigma(z)) = z * (1 - z)
        dz = dout * z * (1 - z)

        # Gradient of weights (dL/dW) = dL/dz * dz/d(s) * d(s)/dw
        dW = np.dot(X.T, dz)
        return dW
