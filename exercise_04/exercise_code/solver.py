...
class Solver(object):
    ...
    def _step(self):
        ...
        # 2. Compute the loss and gradients w.r.t output of the model
        loss = loss_func(predictions, y_train)

        # 3. Backward pass: Compute the gradient of the loss w.r.t the model's parameters
        loss_grad = loss_func.backward(predictions, y_train)  # dL/dz
        grad = model.backward(loss_grad)

        # 4. Update model parameters: Use the optimizer to adjust the model's parameters based on the gradients
        opt.step(grad)
